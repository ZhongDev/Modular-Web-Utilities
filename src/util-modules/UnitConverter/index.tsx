...
const UnitConverter: React.FC = () => {
  ...
  const [sourceUnit, setSourceUnit] = useState<EnergyUnit | DataUnit | DistanceUnit | WeightUnit | VolumeUnit>('kJ');
  const [targetUnit, setTargetUnit] = useState<EnergyUnit | DataUnit | DistanceUnit | WeightUnit | VolumeUnit>('kcal');
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (category === 'energy') {
        ...
        const sourceInJoules = numValue * ENERGY_UNITS[sourceUnit as EnergyUnit].factor;
        convertedValue = sourceInJoules / ENERGY_UNITS[targetUnit as EnergyUnit].factor;
      } else if (category === 'data') {
        // Convert through bits as base unit
        const sourceInBits = numValue * DATA_UNITS[sourceUnit as DataUnit].factor;
        convertedValue = sourceInBits / DATA_UNITS[targetUnit as DataUnit].factor;
      } else if (category === 'distance') {
        // Convert through meters as base unit
        const sourceInMeters = numValue * DISTANCE_UNITS[sourceUnit as DistanceUnit].factor;
        convertedValue = sourceInMeters / DISTANCE_UNITS[targetUnit as DistanceUnit].factor;
      } else if (category === 'weight') {
        // Convert through grams as base unit
        const sourceInGrams = numValue * WEIGHT_UNITS[sourceUnit as WeightUnit].factor;
        convertedValue = sourceInGrams / WEIGHT_UNITS[targetUnit as WeightUnit].factor;
      } else if (category === 'volume') {
        // Convert through liters as base unit
        const sourceInLiters = numValue * VOLUME_UNITS[sourceUnit as VolumeUnit].factor;
        convertedValue = sourceInLiters / VOLUME_UNITS[targetUnit as VolumeUnit].factor;
      } else {
      ...
      const formattedResult = convertedValue < 0.001
  ...
  const getCurrentUnits = () => {
    switch (category) {
      ...
      case 'distance': return DISTANCE_UNITS;
      case 'weight': return WEIGHT_UNITS;
      case 'volume': return VOLUME_UNITS;
      default: return ENERGY_UNITS;
    }
  };
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <p className="text-gray-600 mb-8">
        Convert between different units of measurement across multiple categories in real-time. 
        Supports Energy, Data Storage, Distance, Weight, and Volume units with high precision calculations.
      </p>
        ...
        <div className="mb-6">
          ...
          <div className="hidden md:grid grid-cols-5 gap-3 items-end">
            ...
            <div className="col-span-2">
              <label htmlFor="source-unit" className="block text-sm font-medium text-gray-700 mb-2">
                From Unit
              </label>
              <select
                id="source-unit"
                value={sourceUnit}
                onChange={(e) => setSourceUnit(e.target.value as EnergyUnit | DataUnit | DistanceUnit | WeightUnit | VolumeUnit)}
                className="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              >
            ...
            {/* Target Unit */}
            <div className="col-span-2">
              <label htmlFor="target-unit" className="block text-sm font-medium text-gray-700 mb-2">
                To Unit
              </label>
              <select
                id="target-unit"
                value={targetUnit}
                onChange={(e) => setTargetUnit(e.target.value as EnergyUnit | DataUnit | DistanceUnit | WeightUnit | VolumeUnit)}
                className="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              >
                {Object.entries(currentUnits).map(([key, unit]) => (
                  <option key={key} value={key}>
          ...
          <div className="md:hidden space-y-4">
            {/* Source Unit */}
            <div>
              <label htmlFor="source-unit-mobile" className="block text-sm font-medium text-gray-700 mb-2">
                From Unit
              </label>
              <select
            ...
            {/* Target Unit */}
            <div>
              <label htmlFor="target-unit-mobile" className="block text-sm font-medium text-gray-700 mb-2">
                To Unit
              </label>
              <select
        ...
        {result && (
          <div>
            ...
            <div className="p-4 rounded-md border text-lg font-mono bg-green-50 border-green-200 text-green-800">
              <span>
                <strong>{inputValue} {sourceUnit}</strong> = <strong>{result} {targetUnit}</strong>
              </span>
            </div>
          </div>
        )}
      ...
      <div className="mt-8 grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div className="bg-blue-50 rounded-lg p-4">
          <h3 className="text-lg font-medium text-blue-900 mb-2">⚡ Energy Units</h3>
          <p className="text-blue-700 text-sm">
            Converts between Joules, calories, BTU, and watt-hours. Useful for physics 
            calculations, nutrition planning, and energy consumption analysis.
          </p>
        </div>

        <div className="bg-green-50 rounded-lg p-4">
          <h3 className="text-lg font-medium text-green-900 mb-2">💾 Data Storage Units</h3>
          <p className="text-green-700 text-sm">
            Handles both decimal (1000-based) and binary (1024-based) data units. 
            Perfect for file sizes, storage capacity, and bandwidth calculations.
          </p>
        </div>

        <div className="bg-purple-50 rounded-lg p-4">
          <h3 className="text-lg font-medium text-purple-900 mb-2">📏 Distance Units</h3>
          <p className="text-purple-700 text-sm">
            Converts between metric and imperial distance measurements including meters, 
            kilometers, feet, miles, and nautical miles.
          </p>
        </div>

        <div className="bg-orange-50 rounded-lg p-4">
          <h3 className="text-lg font-medium text-orange-900 mb-2">⚖️ Weight Units</h3>
          <p className="text-orange-700 text-sm">
            Handles metric and imperial weight conversions including grams, kilograms, 
            pounds, ounces, stones, and tons.
          </p>
        </div>

        <div className="bg-cyan-50 rounded-lg p-4">
          <h3 className="text-lg font-medium text-cyan-900 mb-2">🧊 Volume Units</h3>
          <p className="text-cyan-700 text-sm">
            Converts between metric and imperial volume measurements including liters, 
            cubic meters, gallons, cups, and fluid ounces.
          </p>
        </div>

        <div className="bg-gray-50 rounded-lg p-4 lg:col-span-1">
          <h3 className="text-lg font-medium text-gray-900 mb-2">🔧 More Categories</h3>
          <p className="text-gray-700 text-sm">
            This converter supports comprehensive unit conversions across multiple 
            measurement categories with high precision calculations.
          </p>
        </div>
      </div>
